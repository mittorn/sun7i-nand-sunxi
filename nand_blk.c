...
//#ifdef __LINUX_NAND_SUPPORT_INT__
  ...
  { 
	...
	int ratio = 0;
	ret = script_parser_fetch("nand_para","good_block_ratio", &ratio, sizeof(int));
	if(ret)printk("nand init fetch nand_good_block_ratio failed\n");
	if(ratio>0)NAND_SetValidBlkRatio(ratio);
...
{
	printk("[NAND] nand_flush\n");
	nand_flush(NULL);
	printk("[NAND] nand_blk_unregister\n");
	nand_blk_unregister(&mytr);
	#ifdef NAND_CACHE_RW
	printk("[NAND] nand_cacheclose\n");
		NAND_CacheClose();
	#endif
	printk("[NAND] lml_exit\n");
	LML_Exit();
	printk("[NAND] fmt_exit\n");
	FMT_Exit();
	printk("[NAND] PHY_exit\n");
	PHY_Exit();
}
...
void nand_exit(void)
{
	...
	printk("[NAND]nand driver : driver unregistered\n");
	//platform_device_unregister(&nand_device);
	printk("[NAND]nand driver : exiting blk layer\n");
	exit_blklayer();
	printk("[NAND]nand driver : blklayer exited\n");
